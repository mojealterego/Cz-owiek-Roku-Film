# CHARACTER CAST STATUS — CZŁOWIEK ROKU

Centralny indeks referencji postaci. Dokument rozdziela **REFERENCE LOCK**, **SOURCE/ROLE LOCK** oraz elementy jeszcze nieustalone.

## LOCKED VISUAL REFERENCES

| Postać | Status | Ustalone informacje |
|---|---|---|
| Andrzej | LOCKED / MASTER | 42 lata; górnik; fotograf; pełna referencja wizualna; pełna mapa tatuaży; chronologia tatuażu pleców. |
| Weronika | LOCKED | Referencja wizualna z dostarczonych zdjęć; partnerka/żona Andrzeja; jej portret jest pierwotnym tatuażem na plecach Andrzeja. |
| Zuzia | LOCKED | 8 lat; córka Andrzeja; trzy dostarczone zdjęcia jako primary visual reference. |
| Wiktoria | LOCKED | Wiktoria; dostarczone zdjęcia jako primary visual reference; jej portrety stanowią centralny temat fotograficznego "Projektu W." po hospitalizacji Andrzeja. |
| Paulina | LOCKED | Postać drugoplanowa; rozdział 59; lodziarnia; dostarczone zdjęcia jako visual reference. |
| Karol | LOCKED | 30 lat; sąsiad Andrzeja; dostarczone zdjęcie; charakterystyczne okulary, krótka fryzura, krępa sylwetka, tatuaż prawej ręki. |
| Sebastian | LOCKED | 24 lata; młodszy brat Karola; dostarczone zdjęcie; odrębna ciągłość wizualna od Karola. |
| Julka | LOCKED | Postać drugoplanowa/epizodyczna; dostarczone zdjęcie jako primary visual reference. |
| Adrian | LOCKED VISUAL ONLY | Dostarczone zdjęcie jako primary visual reference. Rola narracyjna wymaga źródła. |
| Tomek | LOCKED | Tatuator; dostarczone zdjęcie jako primary visual reference; powiązany z etapem transformacji tatuażu Andrzeja. |
| Żona Tomka | REFERENCE ONLY / OFF-SCREEN | Dostarczone zdjęcie; użytkownik potwierdził, że jest żoną Tomka. Obecnie nie występuje w filmie. |
| Bożena | LOCKED | Dostarczone zdjęcie jako primary visual reference; na referencji stoi obok Andrzeja w przestrzeni wystawienniczej. |

## RELATIONSHIP LOCKS

- Andrzej ↔ Weronika — centralna relacja konfliktu.
- Andrzej ↔ Zuzia — ojciec/córka.
- Karol ↔ Sebastian — starszy/młodszy brat.
- Tomek ↔ Żona Tomka — małżeństwo.
- Andrzej ↔ Bożena — wspólna obecność w dostarczonej fotografii; charakter relacji poza tym kadrem wymaga źródła.

## PROJECT W LOCK

- Wiktoria jest wizualnie zablokowana jako jedna, spójna postać.
- Jej fotografie są przedmiotem/tematem "Projektu W." po hospitalizacji Andrzeja.
- Wystawa stanowi odwrócenie oczekiwania publiczności: zamiast oczekiwanej kontynuacji narracji Weroniki pojawiają się portrety Wiktorii.
- Nie tworzyć różnych wersji tożsamości Wiktorii dla różnych ujęć.

## CRITICAL CONTINUITY LOCK — ANDRZEJ BACK TATTOO

### BEFORE TRANSFORMATION
Plecy Andrzeja = **portret Weroniki**.

### DURING TRANSFORMATION
Tomek jest związany z pracą tatuatorską/przekształceniem tatuażu.

### AFTER TRANSFORMATION
Dawny portret Weroniki zostaje przekształcony w dużą kobiecą kompozycję inspirowaną Santa Muerte:
- intensywnie niebieskie włosy;
- świecące/wyraziste akcenty oczu: fiolet, pomarańcz, żółć;
- ornamentyka Santa Muerte;
- czerwone róże;
- geometryczne linie i ink splashes.

**ABSOLUTNY ZAKAZ:** późniejsza wersja Santa Muerte nie może pojawić się przed momentem transformacji.

## PHOTO REFERENCE RULE

Zdjęcia są referencją wizualną. Nie są automatycznie źródłem:
- biografii;
- osobowości;
- zawodu;
- relacji;
- dialogów;
- scenografii;
- garderoby całego filmu;
- wydarzeń.

Element widoczny na zdjęciu, ale niepotwierdzony źródłem, pozostaje kontekstem fotografii.

## TOMEK WEDDING — CURRENT STATUS

**STATUS: PROPOSED ADAPTATION — NOT CANONICAL**

Pomysł użytkownika:
- dodać scenę wesela Tomka;
- Andrzej jest fotografem na weselu;
- wykonuje obsługę fotograficzną wesela;
- fotografia weselna stanowi sposób rozliczenia za tatuaż wykonany przez Tomka;
- żona Tomka może dzięki temu pojawić się naturalnie w filmie;
- scena łączy zawód Andrzeja z jego przemianą poprzez tatuaż.

### Canon control
Nie dodawać tej sceny do oficjalnego `SCENE_MAP.md`, runtime ani shot listy, dopóki użytkownik nie zatwierdzi jej jako elementu filmu.

### Nadal nieustalone
- dokładna data wesela;
- miejsce;
- umiejscowienie w chronologii;
- czas ekranowy;
- przebieg sceny;
- dialogi;
- dokładny mechanizm rozliczenia;
- czy scena ostatecznie wchodzi do filmu.

## NEXT CASTING TARGETS

Po zakończeniu obecnej serii reference lock należy wrócić do pozostałych postaci wynikających ze scenariusza/książki, zaczynając od postaci o największej liczbie wystąpień i znaczeniu dramaturgicznym.

Priorytet nie oznacza jeszcze visual locku — tylko kolejność zbierania referencji.
